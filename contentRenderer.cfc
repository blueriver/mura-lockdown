...
<cfcomponent extends="mura.content.contentRenderer" output="false">
	...
	<cffunction name="setDynamicContent" returntype="string" output="false">
		...
		<cfset var tempValue="">
		<cfset var pos = 0>
		<cfset var allowedExpressions = this>
		<cfset var expression = "">
		<cfset var trimmedExpression = "">
		...
		<cfloop condition="#finder.len[1]#">
			<cftry>
				...
				<cfif left(tempValue,3) eq "[m]">
					<cfset expression = mid(tempValue, 4, len(tempValue)-7)>
				<cfelse>
					<cfset expression = mid(tempValue, 7, len(tempValue)-13)>
				</cfif>

				<cfset pos = find('(', expression)>
				<cfif pos gt 1>
					<cfset trimmedExpression = left(expression, pos-1)>
					<cfloop collection="#allowedExpressions#" item="i">
						<cfif i eq trimmedExpression>
							<cfset tempValue=evaluate("##" & expression & "##")>
							<cfset allowed = true>
							<cfbreak>
						</cfif>
					</cfloop>
				...
				<cfif not isDefined("tempValue") or not isSimpleValue(tempValue) or not allowed>
					<cfset tempValue="">
				</cfif>
