<cfcomponent extends="mura.cfobject" output="false">
	<cffunction name="onGlobalRequestStart" access="public" output="false" returntype="any">
		...
		<cfif not application.configBean.getCurrentUser().isSuperUser() and listFindNoCase("cSettings.editPlugin,cSettings.deployPlugin,cSettings.updatePlugin,cSettings.updatePluginVersion", $.event('muraAction'))>
			...
			<cfsavecontent variable="returnVar">
				<script>
					$(document).ready(function(){
						$('#tabModules .fieldset').html('<div class="control-group"><div class="alert alert-error">This install does not support configuration of modules.</div></div>')
					});
				</script>
			</cfsavecontent>
		</cfif>
